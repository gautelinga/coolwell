...
ET = df.FiniteElement("Lagrange", mesh.ufl_cell(), 2)
...
one = df.interpolate(df.Constant(1.), W.sub(2).collapse())
...
qy_ref = args.kappa * (args.Ttop-args.Tbtm) / (args.Lx+args.H) * args.Lx
Nu = qy/qy_ref
...
subfolder = "{}/Re{}_b{}".format(folder, Re, args.b)
if rank == 0 and not os.path.exists(folder):
    os.makedirs(subfolder)

with df.HDF5File(mesh.mpi_comm(),
                 "{}/flow.xdmf".format(subfolder), "w") as h5f:
    h5f.write(w_, "w")
